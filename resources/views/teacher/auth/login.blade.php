<x-teacher-layout title="Teacher login">
    <h1 class="mb-2 text-center text-2xl font-bold text-[#7D3C98]">Teacher login</h1>
    <p class="mb-8 text-center text-sm text-gray-600">Log in to your teacher account.</p>

    <form method="POST" action="{{ route('teacher.login.store') }}" class="space-y-6">
        @csrf
    ...
    @if ($registrationOpen)
        <p class="mt-6 text-center text-sm text-gray-600">
            ...
            <a href="{{ route('teacher.register') }}" class="font-medium text-[#7D3C98] underline-offset-4 hover:underline">Create account</a>
        </p>
    @endif
</x-teacher-layout>
